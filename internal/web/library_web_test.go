...
func TestLibraryAndTrashPagesRender(t *testing.T) {
	...
	out := renderString(t, libraryPage(buildLibraryView(files), "csrf-tok", true, []string{"/data/models"}, "dark", "files", nil, false, nil, NSFWBlur))
	...
	tout := renderString(t, trashPage(batches, "csrf-tok", "dark", NSFWBlur))
...
func TestScanFormRendersPathsInput(t *testing.T) {
	...
	sources := renderString(t, libraryPage(buildLibraryView(nil), "csrf-tok", true, nil, "dark", "sources", nil, false, nil, NSFWBlur))
	for _, want := range []string{
	...
	files := renderString(t, libraryPage(buildLibraryView(nil), "csrf-tok", true, []string{"/data/x"}, "dark", "files", nil, false, nil, NSFWBlur))
...
func TestScanFormRendersPersistedSelection(t *testing.T) {
	out := renderString(t, libraryPage(buildLibraryView(nil), "csrf-tok", true, []string{"/data/loras"}, "dark", "sources", nil, false, nil, NSFWBlur))
...
func TestScanFormOmitsPathsInputWhenNotAllowed(t *testing.T) {
	...
	sources := renderString(t, libraryPage(buildLibraryView(nil), "csrf-tok", false, nil, "dark", "sources", nil, false, nil, NSFWBlur))
	...
	files := renderString(t, libraryPage(buildLibraryView(nil), "csrf-tok", false, nil, "dark", "files", nil, false, nil, NSFWBlur))
...
func TestFilesTabGatedOnAddedDir(t *testing.T) {
	for _, allowExtra := range []bool{true, false} {
		...
		empty := renderString(t, libraryPage(buildLibraryView(nil), "csrf", allowExtra, nil, "dark", "files", nil, false, nil, NSFWBlur))
		...
		withDir := renderString(t, libraryPage(buildLibraryView(nil), "csrf", allowExtra, []string{"/some/install/dir"}, "dark", "files", nil, false, nil, NSFWBlur))
