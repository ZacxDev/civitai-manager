...
//
// The value was #6b7280 (4.79:1 on the body) until a real axe run showed the
// token ALSO lands on the 12%/14% color-mix tint the `light` button variant
// paints under it, where #6b7280 was only 4.02:1. #5f6672 clears AA on the body
// AND on both tints; the exact ratios are recomputed from the CSS by
// contrast_web_test.go.
func TestLightThemeDimmedTextOverride(t *testing.T) {
	css := appCSS(t)
	if !strings.Contains(css, `[data-theme="light"] {`) ||
		!strings.Contains(css, "--civitai-color-text-dimmed: #5f6672;") {
		t.Error("app.css must override --civitai-color-text-dimmed to #5f6672 under [data-theme=\"light\"] (3.29:1 -> 5.74:1)")
	}
	// The ratios are the whole point of the deviation — keep them recorded.
	for _, want := range []string{"3.29:1", "5.74:1"} {
