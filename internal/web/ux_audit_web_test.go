...
func TestNoUndefinedThemeTokens(t *testing.T) {
	...
	// Every --civitai-* token app.css consumes must actually exist upstream.
	...
	for _, m := range tokenRe.FindAllStringSubmatch(css, -1) {
		...
		if !strings.Contains(string(theme), m[1]+":") {
			t.Errorf("app.css uses %s, which civitai-theme.css never defines — it will always fall through to its literal fallback", m[1])
		}
	}
}
...
func TestLongUntrustedStringsCanBreak(t *testing.T) {
	...
	cases := map[string]string{
		...
		"workflow resources": renderString(t, workflowDetailPage(
			...
			"{}", "csrf", "dark", NSFWBlur, nil, false, workflowResolver{})),
