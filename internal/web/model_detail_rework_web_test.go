...
// egressNoteFragment is the sentence that used to sit under the import button.
// It is asserted ABSENT rather than imported as a constant, because the constant
// itself was deleted — the test would otherwise silently stop checking anything
// if someone reintroduced the note under a new name.
const egressNoteFragment = "Downloads the workflow zip from civitai.com using your token"

// TestImportButtonCarriesNoEgressNoteAnywhere pins the decision that the note is
// gone from EVERY surface and EVERY form.
//
// This is deliberately stricter than "no paragraph": an earlier revision kept the
// sentence alive in the button's title/aria-label so the disclosure survived for
// screen-reader users. That was explicitly rejected — "remove everywhere means
// everywhere" — so attributes are checked too.
//
// The accepted consequence, recorded here so it is not rediscovered as a bug: the
// model DETAIL page has no page blurb, so it now carries NO inline statement that
// importing sends your CivitAI token to civitai.com. The Discover page still
// explains it in its own blurb; the detail page does not.
func TestImportButtonCarriesNoEgressNoteAnywhere(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			if strings.Contains(out, egressNoteFragment) {
				t.Errorf("the egress note must not appear in body text OR any attribute:\n%s", out)
			}
			for _, attr := range []string{"title=", "aria-label="} {
				if strings.Contains(out, attr) && strings.Contains(out, "civitai.com using your token") {
					t.Errorf("the egress note must not survive in %s:\n%s", attr, out)
				}
			}
