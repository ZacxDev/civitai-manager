...
## Current state
- **Latest release: v0.1.95** (`main` @ `47f09c8`), clean, `HEAD == origin/main`. **Tarball verified** — `checksums.txt` OK, binary runs `0.1.95 (commit 47f09c8…)`, `gh attestation verify` on the TARBALL exits 0.
- `go.mod`/`go.sum` **untouched across the entire v0.1.84–v0.1.95 run** — so `vendorHash` has needed no re-verification. Checked every release, not assumed.
- Dogfood **v0.1.95 on `:8972`** — the RELEASED binary, at `…/c54c3347-…/scratchpad/dogfood/cm` (moved off the old dead session's scratchpad). Live-verified on the RELEASED binary: `h1` reads Overview (Dashboard gone), the Library menu is a `popover` (no `<details>` in the nav), `/disks` 200.
