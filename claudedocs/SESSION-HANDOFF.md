...
5. **`devrc` changes are LIVE but still UNCOMMITTED** — `home-manager switch` was run 2026-07-29, so the updated `claude/RULES.md`, `claude/commands/audit-pr.md` and `claude/commands/verify-agent.md` are in effect (`scripts/browser-bridge/SKILL.md` is a direct symlink and was live immediately). They are **still uncommitted in `~/workspace/devrc`** — commit them there or the next switch from a clean tree loses them.
